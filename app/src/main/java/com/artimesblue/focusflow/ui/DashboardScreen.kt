    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                AssistChip(onClick = { onQuickAdd(1) }, label = { Text("+1") }, modifier = Modifier.semantics { contentDescription = context.getString(R.string.add_amount_to_habit, 1, habit.unit, habit.name) })
                AssistChip(onClick = { onQuickAdd(5) }, label = { Text("+5") }, modifier = Modifier.semantics { contentDescription = context.getString(R.string.add_amount_to_habit, 5, habit.unit, habit.name) })
                AssistChip(onClick = { onQuickAdd(10) }, label = { Text("+10") }, modifier = Modifier.semantics { contentDescription = context.getString(R.string.add_amount_to_habit, 10, habit.unit, habit.name) })
            }
        }
    }
}
